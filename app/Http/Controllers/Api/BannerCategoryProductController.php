...
class BannerCategoryProductController extends Controller
{
  public function bannerCategoryProduct(Request $request)
  {
    ...
    {
      $getBannerCategoryData= Banner::select('banners.*','mastercategory.master_category_name','mastercategory.master_category_image','maincategory.main_category_name','maincategory.main_category_image','category.category_name','subcategory.sub_category_name','category.category_image','subcategory.sub_category_image')
      ...
      if($getBannerCategoryData['mastercategory_id']!=0 && $getBannerCategoryData['maincategory_id']==0  && $getBannerCategoryData['category_id']==0 && $getBannerCategoryData['subcategory_id']==0 )
      {
        ...
        {
        $mainCategory=MainCategory::where('id','=',$masterMainCategoryDataValue['maincategory_id'])->get()->toArray(); 
      
        if($mainCategory != 0)
        {
          $mainCategoryData = ProductMainCategory::where('maincategory_id','=',$masterMainCategoryDataValue['maincategory_id'])->get()->toArray(); 
          $data = array();
          foreach($mainCategoryData as $mainCategoryDataValue)
          {
            $getProductmain = Product::where('id','=',$mainCategoryDataValue['product_id'])->limit(10)->get()->toArray();
      ...
      if($getBannerCategoryData['mastercategory_id']!=0 && $getBannerCategoryData['maincategory_id']!=0  && $getBannerCategoryData['category_id']==0 && $getBannerCategoryData['subcategory_id']==0 )
      {
        ...
        foreach($mainCategoryCategoryData as $mainCategoryCategoryDataValue)
        {
          ...
          { 
            $productCategoryData = ProductCategory::where('category_id','=',$mainCategoryCategoryDataValue['category_id'])->get()->toArray(); 
            $data = array();
            foreach($productCategoryData as $productCategoryDataValue)
            {
              $getProductCategory = Product::where('id','=',$productCategoryDataValue['product_id'])->limit(10)->get()->toArray();
        ...
        if($flag==0)
        {
          $mainCategoryCategoryData=MainCategory::where('id','=',$getBannerCategoryData['maincategory_id'])->get()->toArray(); 
          foreach($mainCategoryCategoryData as $mainCategoryCategoryDataValue)
          {
            $productMainCategoryData = ProductMainCategory::where('maincategory_id','=',$getBannerCategoryData['maincategory_id'])->get()->toArray(); 
            $data = array();
            foreach($productMainCategoryData as $productMainCategoryDataValue)
            {
              $getProductCategory = Product::where('id','=',$productMainCategoryDataValue['product_id'])->limit(10)->get()->toArray();
              foreach($getProductCategory as $getCategoryValue){
                $productImage = ProductsImage::select()->where('product_id',$getCategoryValue['id'])->get()->toArray();
      ...
      if($getBannerCategoryData['mastercategory_id']!=0 && $getBannerCategoryData['maincategory_id']!=0  && $getBannerCategoryData['category_id']!=0 && $getBannerCategoryData['subcategory_id']==0 )
      {
        ...
        {
          $subCategory=SubCategory::where('id','=',$categorySubCategoryDataValue['subcategory_id'])->get()->toArray();
          if($subCategory != 0)
          {
            $subCategoryData = ProductSubCategory::where('subcategory_id','=',$categorySubCategoryDataValue['subcategory_id'])->get()->toArray(); 
            $data = array();
            foreach($subCategoryData as $subCategoryDataValue)
            {
              $getSubProduct = Product::where('id','=',$subCategoryDataValue['product_id'])->limit(10)->get()->toArray();
        ...
        if($flag==0)
        {
          $categoryData=Category::where('id','=',$getBannerCategoryData['category_id'])->get()->toArray(); 
          foreach($categoryData as $mainCategoryCategoryDataValue)
          {
            $productCategoryData = ProductCategory::where('category_id','=',$getBannerCategoryData['category_id'])->get()->toArray(); 
            $data = array();
            foreach($productCategoryData as $productCategoryDataValue)
            {
              $getProductCategory = Product::where('id','=',$productCategoryDataValue['product_id'])->limit(10)->get()->toArray();
      ...
      if($getBannerCategoryData['mastercategory_id']!=0 && $getBannerCategoryData['maincategory_id']!=0  && $getBannerCategoryData['category_id'] !=0 && $getBannerCategoryData['subcategory_id']!=0 )
      {
        $subCategoryAllData=SubCategory::where('id','=',$getBannerCategoryData['subcategory_id'])->get()->toArray(); 
        foreach($subCategoryAllData as $key=>$subCategoryAllDataValue)
        {
          $productSubCategoryData = ProductSubCategory::where('subcategory_id','=',$getBannerCategoryData['subcategory_id'])->get()->toArray(); 
          $data = array();
          foreach($productSubCategoryData as $productSubCategoryDataValue)
          {
            $getProductCategory = Product::where('id','=',$productSubCategoryDataValue['product_id'])->limit(10)->get()->toArray();
